...
#Important constants
a = 0               #Lower bound for shifted Chebyshev polynomials
b = 1               #Upper bound for shifted Chebyshev polynomials
...
def LCDMSolver(n: int, M: int, Wm: float):
    ...
    #w present for future generalisation: use w = -1
    def Dminus(x: float, H0: int, Wm: int, w: int):
        #Returns D_- (negative growing mode, equal to conformal Hubble parameter)
        return H0*x*np.sqrt(Wm*x**(-3) + (1 - Wm)*x**(-3*(1 + w)))
    ...
    def fminus(x: float, Wm: float, w: float):
        ...
        y = ((1 - Wm)/Wm)*x**(-3*w)
        return -1.5 - 1.5*w*y*(1+y)**(-1)

    def fplus(x: float, Wm: float, w: float):
        #Returns f_+ (logarithmic growth rate of growing mode)
        y = ((1 - Wm)/Wm)*x**(-3*w)
        return 1 - 1.5*w*y*(1+y)**(-1) + (45*w*y*(1+y)**(0.5)/(10 - 12*w))*(x/Dplus(x,Wm,w))*hyp2f1(2.5,(6*w - 5)/(6*w), (12*w - 5)/(6*w),-y) - 15*w*(1+w)*y*(1+y)**(0.5)*(x/Dplus(x,Wm,w))*((5 - 6*w)**(-1)*hyp2f1(1.5,1 - 5*(6*w)**(-1),2 - 5*(6*w)**(-1),-y ) + 3*y*(24*w - 10)**(-1)*hyp2f1(2.5,2 - 5*(6*w)**(-1),3 - 5*(6*w)**(-1),-y))
    ...
    if n == 1:
        ...
        """
        w = -1 #Dark energy equation of state parameter, for future generalisation
